...
def hefty_hogs(player_score, opponent_score):
    ...
    sum, num = player_score, opponent_score

    while num > 0:
        digit, num = num % 10, num // 10
        sum = digit_fn(digit)(sum)
...
def hog_pile(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    p_digit, o_digit = player_score % 10, opponent_score % 10
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    hh_score = hefty_hogs(score, opponent_score)

    if hog_pile(score + hh_score, opponent_score):
        return 0
